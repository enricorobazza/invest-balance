...
class EvolutionViews():
  ...
  def evolution_chart2(request):
    if(request.user.is_anonymous):
      user = User.objects.get(pk=1)
    else:
      user = request.user
      # return redirect('/login')
    assets = Asset.objects.filter(user=user).values('code')
    first_date = list(AssetPurchase.objects.filter(asset__user=user).order_by('date').values('date'))[0]["date"]

    return render(request, 'Charts/evolution.html', {'assets': list(assets), 'first_date': first_date})
  ...
  def get_stock_historical_yield(request, first_date, code):
    if(request.user.is_anonymous):
      user = User.objects.get(pk=1)
    else:
      user = request.user

    ## asset_purchases accumulated amount by month
    asset_purchases = AssetPurchase.objects.filter(asset__code=code, asset__user=user).values(year= ExtractYear('date') ,month=ExtractMonth('date')).annotate(
      accumulated_amount=Window(
        expression=Sum('amount'),
        order_by=[ExtractYear('date').asc(),ExtractMonth('date').asc()]
      ),
    ...
    timestamp_beginning = time.mktime((datetime.date(datetime.strptime(first_date, "%Y-%m-%d"))-timedelta(days=31)).timetuple())

    response = urllib.request.urlopen("https://query1.finance.yahoo.com/v7/finance/download/%s?period1=%s&period2=%s&interval=1mo&events=history&includeAdjustedClose=true"%(code, int(timestamp_beginning), int(time.time())))
    ...
    return JsonResponse({"prices": month_prices})